...
> 当前版本 **v5.7「天气重构」** — 统一农场与远征两套天气为 7 种连续天气（状态+云量/降水/雾/风/积雪强度，8–15s 平滑过渡）、世界空间 3 层雨（视差/水花/水洼反光/湿脚印）、实体闪电劈在世界坐标（落雷前 1.2–1.8s 红圈预警、可走位躲避、点燃油桶连锁、距离雷声）、雾用 FBM 噪声与战争迷雾/视野真正融合、降雪积雪与寒冷值（>40 掉能量、>92 掉血）、天气双向影响玩法（雨弱火强电/雾双向伏击/雪需抗寒）、新增集雨桶与避雷针、顶部天气预报 HUD；数值表升级为 32 sheet《游戏数据表_v5.7.xlsx》。
> 上一版本 **v5.6「家园精修」** — R 键道具径向转盘、消耗品（含火把）限带 6 种每种 5 个、火把限时燃烧自动续燃、火绒草产火把、强化卡改卡牌工坊永久升级且 5 合 1、一键种植/收成熟/除害、雨雷斜纹柔化；更早：v5.5「家园联动」 — 加工工坊成品全部接入远征消耗品、火把照明按难度分级且无火把视野极小、农场实时家园指挥台、美观度增益显性化、天气屏幕特效、撤离卡死修复、边界迷雾裁剪；v5.4「荒野校准」：难度两维重做、12 Boss 软狂暴、兽潮指数化、千局 bot 校准、经济闭环 7 次撤离满级；v5.0「荒野觉醒」：100 级修行台 × 16 技能 × 远征档案 × 14 种具体资源 × 12 Boss/24 地图 × 105 张统一写实图标

融合 QQ 农场经营 + 卡牌收集 + 搜打撤远征的 2D 网页游戏。

## 玩法

- **农场经营**：种植 24 种作物（战斗型/季节型/稀有型/修为型）、4 阶段生长、4 品质变异、作物组合、加工工坊（16 配方闭环）、14 种温室作物、**修行台（角色 1–100 级升级与技能装备）**
...
- **地图表现**：分层地形（土路/泥地/水洼/密林/废墟）、三层视差远景、昼夜循环与火把照明、**连续天气系统（世界空间雨雪/实体闪电预警/FBM 雾与视野融合/积雪寒冷）**、空间叙事地标、相机震屏与惯性
...
js/
  ...
  world-fx.js            地图真实感模块（后处理/密林/废墟/远景/光照/叙事道具/相机）
  weather-system.js      v5.7 统一连续天气（强度过渡/权重/雨雪雾渲染/落雷预警/寒冷/天气音频）
