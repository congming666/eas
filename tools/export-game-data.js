...
async function main() {
  ...
  const data = await page.evaluate(() => {
    ...
    return {
      ...
      deployPlants: pick(CONFIG.deployPlants),
      cropToDeploy: pick(CONFIG.cropToDeploy),
      maps: pick(CONFIG.maps),
      monsters: pick(CONFIG.monsters),
      plants: pick(CONFIG.plants),
      decorations: (typeof FarmDecorationSystem !== 'undefined' && FarmDecorationSystem.items) ? pick(FarmDecorationSystem.items) : null,
      torch: { baseFuel: 60, perDifficulty: { casual: 120, normal: 60, hard: 40, nightmare: 30 } },
